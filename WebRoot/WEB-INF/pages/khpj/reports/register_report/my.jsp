...
<body>
	<div class="container mainbody">
		...
		<div class="row-fluid">
			<div class="span10">
				<ul class="breadcrumb">
					...
					<li><span class="divider">|</span> 政绩档案登记表</li>
				</ul>
			</div>
		...
		<ul class="nav nav-tabs">
			<li <c:if test="${empty data}">class="active"</c:if>><a href="#A" data-toggle="tab">政绩档案登记列表</a>
			</li>
			<li <c:if test="${not empty data}">class="active"</c:if>><a href="#B" data-toggle="tab">登记表维护</a>
			</li>
		</ul>
		<div class="tab-content">
					...
					<c:if test="${not empty datas}">
						<ul>
							<c:forEach var="d" items="${datas.result}">
								<li><a href="${ctx}/khpj/reports/register_report/view/${d.registerReportId}.html" target="_blank">${d.reportYear} ${d.name} ${d.duty} 政绩档案登记表</a></li>
							</c:forEach>
						</ul>
					</c:if>
				</div>
			...
			<div class="tab-pane <c:if test="${not empty data}">active</c:if>" id="B">
				<div class="row-fluid">
					<form action="${ctx}/khpj/reports/register_report/save.html" method="post" onsubmit="return confirm('是否提交数据？')">
						<table class="table table-bordered table-condensed">
							<thead>
								<tr>
									<th colspan="6">
										<h3 class="text-center">综治、维稳责任人履行职责情况登记表</h3>
										<div>
											<p class="text-right">
												<select class="span2" name="reportYear">
							...
							<tbody>
								...
								<tr>
									...
									<td colspan="5">
										<p>
											<textarea rows="15" name="workingSituation" class="input-block-level">${data.workingSituation}</textarea>
										</p>
									</td>
								</tr>
								<tr>
									...
									<td colspan="5">
										<p>
											<textarea rows="5" name="auditResult" class="input-block-level">${data.auditResult}</textarea>
										</p>
									</td>
								</tr>
								<tr>
									...
									<td colspan="5">
										<p>
											<textarea rows="3" name="remarks" class="input-block-level">${data.remarks}</textarea>
										</p>
									</td>
								</tr>
							</tbody>
						...
						<div class="row">
							<p class="text-right">
								...
								<input type="hidden" name="registerReportId" value="${data.registerReportId}"/>
								&nbsp;&nbsp;
							</p>
						</div>
					</form>
